  ...
  return (
    <div className={[
      ...
      <div className="mb-4 flex items-start justify-between">
        ...
        {(plan.matchPercentage || plan.score) && (
          <div className="text-right">
            <div className="text-2xl font-bold text-emerald-300">
              {plan.matchPercentage || Math.round(plan.score)}%
            </div>
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible' && isAuthenticated()) {
        console.log('[DashboardPage] Page visible - refreshing data');
        fetchUserData();
      }
    };
    ...
    try {
      ...
      setRecommendations(recoData?.recommendations || []);
        ...
        if (recoError.message?.includes('empty_profile') || recoError.message?.includes('incomplete')) {
          console.log('[DashboardPage] User has empty profile (skipped survey)');
          setRecommendations([]); // Pusta lista rekomendacji
          // Nie ustawiamy error - wyświetlimy specjalny komunikat w UI
        } else {
          console.error('Błąd ładowania rekomendacji:', recoError);
          setError(recoError.message);
        }
      }
    } catch (err) {
                    ...
                    <div>
                      ...
                      {recoPrimary.score && (
                        <div className="flex items-center gap-2 text-blue-300">
                          ...
                          <span className="font-semibold">Dopasowanie: {Math.round(recoPrimary.score)}%</span>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {hasReco ? (
              <div>
                ...
                <div className="grid gap-4">
                  {recommendations
                    .filter(plan => {
                      // 🆕 Nie pokazuj planu, który jest już aktywny
                      const planId = plan.planId || plan.id;
                      const activeId = activePlan?.planId || activePlan?.id;
                      return !activeId || !planId || String(planId) !== String(activeId);
                    })
                    .slice(0, 3)
                    .map((plan, index) => (
                    <PlanCard 
