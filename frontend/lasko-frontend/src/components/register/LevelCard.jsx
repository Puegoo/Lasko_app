...
const LevelCard = ({ formData, updateFormData, onNext, onPrev }) => {
  ...
  const levels = [
    { 
      value: 'poczatkujacy', 
      label: 'Początkujący', 
      description: 'Dopiero zaczynam swoją przygodę z treningiem'
    },
    { 
      value: 'sredniozaawansowany', 
      label: 'Średniozaawansowany', 
      description: 'Trenuję regularnie od kilku miesięcy'
    },
    { 
      value: 'zaawansowany', 
      label: 'Zaawansowany', 
      description: 'Mam duże doświadczenie treningowe'
    }
  ];
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!selectedLevel) {
      alert('Proszę wybrać poziom zaawansowania');
      return;
    }
    onNext();
  };
    ...
    <div className="bg-[#0a0a0a]/95 rounded-3xl shadow-xl p-8 w-full h-[550px] flex flex-col shadow-[0_0_30px_10px_rgba(0,0,0,0.5)] border border-[#222222]">
      <form onSubmit={handleSubmit} className="flex flex-col space-y-6 flex-grow">
        ...
        <div className="max-w-xs mx-auto w-full h-3 bg-gray-800 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-[#0D7A61] to-[#1DCD9F] rounded-full" style={{ width: '50%' }}></div>
        </div>
        
        <div className="text-center mt-8">
          <h2 className="text-white text-2xl font-bold">Jaki jest Twój poziom?</h2>
          <p className="text-white text-lg">Pomoże nam to dobrać odpowiedni plan</p>
        </div>
